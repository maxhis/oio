<script setup lang="ts">
  ...
  const GithubIcon = uiIcons.github;
...
<template>
  <aside class="site-sidebar" :class="{
    ...
    <header class="site-sidebar__header">
      ...
      <div class="site-sidebar__actions">
        <button type="button" class="site-sidebar__toggle site-sidebar__toggle--desktop" @click="emit('toggleSidebar')"
          aria-label="切换侧边栏">
          <MenuIcon :size="18" :stroke-width="2.1" />
        </button>
        <button type="button" class="site-sidebar__toggle site-sidebar__toggle--mobile"
          @click="emit('toggleMobileMenu')" aria-label="关闭菜单">
